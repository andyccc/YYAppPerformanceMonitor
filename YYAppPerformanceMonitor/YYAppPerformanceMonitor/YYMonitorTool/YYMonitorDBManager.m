...
static NSString *const kMonitorDBFPSTableName       = @"YYFPSTable";
static NSString *const kMonitorDBCPUTableName       = @"YYCPUTable";
static NSString *const kMonitorDBMemoryTableName    = @"YYMemoryTable";
static NSString *const kMonitorDBExceptionTableName = @"YYExceptionTable";

// 共享数据库
static sqlite3 *_shared_db = nil;

@interface YYMonitorDBManager()
@property (nonatomic, assign) BOOL hasDBOpened;
@end

@implementation YYMonitorDBManager {
    pthread_mutex_t _mutex;
}

+ (instancetype)sharedManager {
    static YYMonitorDBManager *manager = nil;
...
- (void)insertWithType:(YYAppMonitorDBType)type obj:(__kindof id)obj {
    NSString *sql;
    NSString *appInfo = [YYAppInfo appInfo];
    switch (type) {
        case YYAppMonitorDBTypeFPS:
            sql = [NSString stringWithFormat:kMonitorDBFPSInsertSqlFmt, kMonitorDBFPSTableName, appInfo, ((YYPerformanceModel *)obj).value];
            break;
        case YYAppMonitorDBTypeCPU:
            sql = [NSString stringWithFormat:kMonitorDBCPUInsertSqlFmt, kMonitorDBCPUTableName, appInfo, ((YYPerformanceModel *)obj).value];
            break;
        case YYAppMonitorDBTypeMemory:
            sql = [NSString stringWithFormat:kMonitorDBMemoryInsertSqlFmt, kMonitorDBMemoryTableName, appInfo, ((YYPerformanceModel *)obj).value];
            break;
        case YYAppMonitorDBTypeException: {
            YYException *exc = obj;
...
- (NSArray <__kindof id> *)selectAllRecordWithSql:(NSString *)sql type:(YYAppMonitorDBType)type {
...
__kindof id handle_selected_data(YYAppMonitorDBType type, sqlite3_stmt *stmt) {
    id res;
    if (type == YYAppMonitorDBTypeException) {
...
YYPerformanceModel *
handle_perfomance_selected_data(YYAppMonitorDBType type, sqlite3_stmt *stmt) {
    ...
    YYPerformanceModel *m = [YYPerformanceModel new];
    m.ID = ID;
...
YYException *handle_exceptin_selected_data(sqlite3_stmt *stmt) {
    int ID = sqlite3_column_int(stmt, 0);
    const unsigned char *app_info = sqlite3_column_text(stmt, 1);
    NSString *objcAppInfo = [NSString stringWithCString:(const char *)app_info encoding:NSUTF8StringEncoding];
    ...
    YYException *exc = [YYException new];
    exc.ID = ID;
